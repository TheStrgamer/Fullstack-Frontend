...
import { fetchActiveChats } from '@/services/chatService.ts';
...
onMounted(async () => {
  try {
    ...
    console.log(chats.value[0].name);   
  } catch (error) {
...
const name = computed(() => chats.value.find(chat => chat.id === chatId.value)?.name || 'Chat');
const avatar = computed(() => chats.value.find(chat => chat.id === chatId.value)?.avatar || '');
